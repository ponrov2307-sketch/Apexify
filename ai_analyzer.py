...
def generate_apexify_report(tech_data, role='free'):
    ...
    if role in ['vip', 'pro']:
        ...
        if role == 'pro':
            ...
            เขียนคำแนะนำภาษาไทยให้ฟังเหมือนนักวิเคราะห์ที่คุยกับลูกค้าจริง
            โทนต้องเป็นธรรมชาติ อ่านลื่น ไม่แข็ง ไม่เป็นภาษาหุ่นยนต์ และไม่พูดซ้ำข้อมูลเดิมแบบรายงาน
            แยกเป็น 2 มุมมอง:
            ...
            - หลีกเลี่ยงสำนวนแข็งๆ เช่น "เพื่อประกอบการตัดสินใจ", "ติดตามอย่างใกล้ชิด", "สัญญาณส่วนใหญ่"
            - ใช้ภาษาธรรมชาติแบบ "แถวนี้", "ถ้าจะเข้า", "ถ้าถืออยู่", "ค่อยรอ", "ยังไม่ต้องรีบ"
            - สรุปเหตุผลแบบย่อยง่าย ไม่ไล่ท่อง indicator ซ้ำทุกตัว
            - ความยาวรวมไม่เกิน 4 บรรทัด
            - บรรทัดสุดท้ายให้ใช้รูปแบบนี้เท่านั้น: **🎯 สรุปให้เลย: [ซื้อ / ถือ / รอดู / ขาย] เพราะ [เหตุผลสั้นๆ 1 ประโยค]**
            
            ตัวอย่างโทนที่ต้องการ:
            🏃‍♂️ สายเล่นสั้น: แถวนี้ยังไม่ค่อยน่ารีบเข้า รอดูแรงซื้อกลับใกล้แนวรับก่อน
            🧘‍♂️ สายถือยาว: ภาพใหญ่ยังอ่อนอยู่ ถ้าจะเพิ่มไม้ค่อยรอจังหวะที่นิ่งกว่านี้
            ภาพรวมตอนนี้ยังเป็นฝั่งระวังมากกว่าฝั่งลุย
            🎯 สรุปให้เลย: รอดู เพราะ โมเมนตัมยังไม่พอให้รีบเข้า
            """
        else:
            prompt = f"""
            คุณคือ AI ผู้ช่วยวิเคราะห์หุ้นของ Apexify
            ...
            เขียนภาษาไทยแบบสั้น กระชับ ฟังเป็นธรรมชาติ เหมือนคนอธิบายจริง ไม่ใช่ภาษาหุ่นยนต์
            ถ้าสัญญาณไม่ชัดหรือมีทั้งบวกและลบ ให้สรุปเป็น "รอดู"
            หลีกเลี่ยงคำว่า "หนี", "รับมีด", "ขายเลย", หรือถ้อยคำตื่นตระหนก
            หลีกเลี่ยงสำนวนแข็งๆ เช่น "เพื่อประกอบการตัดสินใจ", "ติดตามอย่างใกล้ชิด"
            ใช้ภาษาธรรมชาติแบบ "ยังไม่ต้องรีบ", "ค่อยรอดู", "ถ้าจะเข้า"
            ความยาวรวมไม่เกิน 3 บรรทัด
            บรรทัดสุดท้ายให้ใช้รูปแบบนี้เท่านั้น: **🎯 สรุปให้เลย: [ซื้อ / ถือ / รอดู / ขาย] เพราะ [เหตุผลสั้นๆ 1 ประโยค]**
            """

        try:
            ai_response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
            report += f"💡 *{_polish_ai_playbook(ai_response.text)}*"
